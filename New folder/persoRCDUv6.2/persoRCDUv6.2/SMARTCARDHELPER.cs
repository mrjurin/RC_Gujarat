...
// Type: persoRCDU.SMARTCARDHELPER
...
namespace persoRCDU
{
  public class SMARTCARDHELPER
  {
    ...
    public bool SCardEstablishContext_T(ref int SCHCONTEXT)
    {
    ...
    public bool SCardRealeaseContext_T(int SCHCONTEXT)
    {
        if (SCardReleaseContext(SCHCONTEXT) == SCARD_S_SUCCESS)
            return true;
        else
            return false;
    }

    public bool Connect_Slected_Reader(int SCHCONTEXT, string READERNAME, ref uint READERHANDEL, ref uint READERACTIVEPROTOCAL, ref string MSGSTATUS)
    {
      if (SMARTCARDHELPER.SCardConnect(SCHCONTEXT, READERNAME, 2U, 3U, out READERHANDEL, out READERACTIVEPROTOCAL) == 0)
      {
        MSGSTATUS = "0:SUCESS";
        return true;
      }
      MSGSTATUS = "Smart Card Connection Error ... ";
      return false;
    }
    ...
    public bool GetCardResponce(uint READERHANDEL, uint ACTIVPROTO, int Len, ref string OUTPUTRESULT)
    {
    ...
    public bool ReadChipSrnoWithExecuteCommand(uint READERHANDEL, uint ACTIVPROTO, ref string CMDRESULT)
    {
      ...
      if (SMARTCARDHELPER.SCardTransmit(READERHANDEL, SendPci, OutputByte, (uint) OutputByte.Length, IntPtr.Zero, numArray, ref RecvLength) == 0)
      {
        ...
        string @string = CMDRESULT.Substring(0, 2).ToString();
        if ((int) ACTIVPROTO == 2 && (int) OutputByte[1] == 164 && (int) OutputByte[3] == 4 && CMDRESULT == "9000")
        {
          CMDRESULT = string.Empty;
          return this.GetCardResponce(READERHANDEL, ACTIVPROTO, (int) byte.MaxValue, ref CMDRESULT);
        }
        if (@string == "61")
        {
    ...
    public bool ExecuteCommand(uint READERHANDEL, uint ACTIVPROTO, string Command, ref string CMDRESULT)
    {
      ...
      if (SMARTCARDHELPER.SCardTransmit(READERHANDEL, SendPci, OutputByte, (uint) OutputByte.Length, IntPtr.Zero, numArray, ref RecvLength) == 0)
      {
        this.ByteToHex(numArray, (int) RecvLength, ref CMDRESULT);
        string @string = CMDRESULT.Substring(0, 2).ToString();
        //check if file allready exist than return true
        if ((int)OutputByte[1] == 224 &&  CMDRESULT == "6A89")
        {
            return true;
            //CMDRESULT = string.Empty;
            //return this.GetCardResponce(READERHANDEL, ACTIVPROTO, (int)byte.MaxValue, ref CMDRESULT);
    ...
    public bool GETMICCARDREADER(ref string ReaderName, ref string ERRMSG)
    {
      try
      {
        ...
        {
          if (this.Connect_Slected_Reader(SCHCONTEXT, READERLIST[index], ref READERHANDEL, ref READERACTIVEPROTOCAL, ref ERRMSG) && this.CHECKMICFILE(READERHANDEL, READERACTIVEPROTOCAL, ref ISMICCARD))
          {
            if (ISMICCARD)
    ...
    public bool READURN(ref string URNNO)
    {
      try
      {
        ...
        for (int index = 0; index < READERLIST.Count; ++index)
        {
          ...
          if (!this.Connect_Slected_Reader(SCHCONTEXT, READERNAME, ref READERHANDEL, ref READERACTIVEPROTOCAL, ref str1))
          {
            URNNO = str1;
    ...
    public bool DecimalStrHexStr(string Input, ref string Output)
    {
      ...
      while (startIndex < Input.Length)
      {
        int int32 = Convert.ToInt32(Input.Substring(startIndex, 2));
        Output += int32.ToString("X").PadLeft(2, '0');
        startIndex += 2;
      }
      return true;
    }

    public bool CHECKBENEFECIARYREADER(ref string URNNO, ref uint READERHANDEL, ref uint ACTIVPROTO)
    {
      try
      {
        READERHANDEL = 0U;
        ...
        {
          string READERNAME = READERLIST[index];
          if (!this.Connect_Slected_Reader(SCHCONTEXT, READERNAME, ref READERHANDEL, ref ACTIVPROTO, ref str1))
          {
            URNNO = str1;
            int num = (int) SMARTCARDHELPER.SCardReleaseContext(SCHCONTEXT);
            return false;
    ...
    public bool HEX2DEC(string HexStr, ref int VALUE)
    {
      try
      {
        ...
        for (; index < length; ++index)
        {
          ...
          {
            int32 = Convert.ToInt32((int) HexStr[index] - 55);
          }
          else
          {
            if ((int) HexStr[index] <= 47 || (int) HexStr[index] >= 58)
              return false;
            int32 = Convert.ToInt32((int) HexStr[index] - 48);
          }
          int num = int32 * Convert.ToInt32(Math.Pow(16.0, (double) (length - (index + 1))));
          VALUE += num;
        }
        return true;
      }
    ...
    public bool WriteOnCurrentSelectedFile(uint ScHandle, uint ScProto, string HexInputData, string StartIndex, ref string ErrMsg)
    {
      try
      {
        ...
        for (; num1 < num3; ++num1)
        {
          string Command = Ins + StartIndex + "FA" + HexInputData.Substring(startIndex, 500).ToString();
          if (!this.ExecuteCommand(ScHandle, ScProto, Command, ref CMDRESULT))
          {
              if(CMDRESULT == "6985")
              {
                  Ins = "00D6";
                  Command = Ins + StartIndex + "FA" + HexInputData.Substring(startIndex, 500).ToString();
        ...
        string Command1 = Ins + StartIndex + num5.ToString("X").PadLeft(2, '0') + HexInputData.Substring(startIndex, num5 * 2).ToString();
        if (this.ExecuteCommand(ScHandle, ScProto, Command1, ref CMDRESULT) || !(CMDRESULT == "9000"))
          return true;
        else if (CMDRESULT == "6985")
        {
            Command1 = "00D6" + StartIndex + num5.ToString("X").PadLeft(2, '0') + HexInputData.Substring(startIndex, num5 * 2).ToString();
            if (this.ExecuteCommand(ScHandle, ScProto, Command1, ref CMDRESULT) || !(CMDRESULT == "9000"))
                return true;
        }
        // ISSUE: explicit reference operation
        ...
        for (; num <= Record; ++num)
        {
          ...
          if (!this.ExecuteCommand(readerHandle, readerProto, Command, ref CMDRESULT))
          {
              if(CMDRESULT == "6985")
                  ...
                  Command = "00DC" + num.ToString("X").PadLeft(2, '0') + "04" + Length.ToString("X").PadLeft(2, '0') + str2;
